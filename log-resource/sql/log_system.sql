...
CREATE TABLE `log_store_record` (
  `id`                INT(11) NOT NULL AUTO_INCREMENT,
  `collected_item_id` INT(11) NOT NULL
  COMMENT '采集器名',
  ...
  `create_time`       DATETIME         DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `fk_store_collected_item` (`collected_item_id`),
  CONSTRAINT `fk_store_collected_item` FOREIGN KEY (`collected_item_id`) REFERENCES `log_collected_item` (`id`)
)
  ENGINE = InnoDB
  AUTO_INCREMENT = 6
  DEFAULT CHARSET = utf8;
