...
import cn.yunyichina.log.service.searcher.util.IndexManagerV1;
 ...
 */
@Service
public class UploadServiceV2 {

    final LoggerWrapper logger = LoggerWrapper.getLogger(UploadServiceV2.class);

    @Autowired
    IndexManagerV1 indexManager;
    ...
    public void uploadFile(MultipartFile file, String applicationName) throws Exception {
        INDEX_ROOT_DIR = config.getConfigItemMap().get(applicationName).getIndexRootDir();
        UPLOADED_LOG_ROOT_DIR = config.getConfigItemMap().get(applicationName).getLogRootDir();
