...
import cn.yunyichina.log.service.searcher.service.SearchServiceV1;
...
public class SearchController {
    ...
    @PostMapping("history")
    public Response historyV1(@RequestBody String json) {
        try {
    ...
    @PostMapping("logs")
    public Response logsV1(@RequestBody String json) {
        try {
    ...
    public Response historyV2(@RequestBody String json) {
        try {
            logger.contextBegin("请求搜索历史日志:" + json);
            SearchCondition condition = JSON.parseObject(json, SearchCondition.class);
            logger.info("obj:" + JSON.toJSONString(condition, true));
    ...
    @PostMapping("logsV2")
    public Response logsV2(@RequestBody String json) {
        try {
            logger.contextBegin("获取可下载的日志:" + json);
            SearchCondition condition = JSON.parseObject(json, SearchCondition.class);
            logger.info("obj:" + JSON.toJSONString(condition, true));
    ...
    public ResponseEntity<byte[]> test(){
        ...
        condition.setCollector(new Collector().setApplication_name("shensan-test"));
