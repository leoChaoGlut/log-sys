...
public class TagService {
    ...
    private TagSet buildTagSet(String applicationName) {
        ...
        Set<KvIndex> kvTagSet = getkvTagSet(applicationName);
    ...
    private Set<String> getKeywordSet(String applicationName) {
        ...
        if (collector == null) {
            ...
            Set<MidCollectorKeyword> midCollectorKeywordSet = keywordIndexMapper.selectMidByCollectorId(collector.getId());
            if (CollectionUtils.isEmpty(midCollectorKeywordSet)) {
                return new HashSet<>();
            } else {
                List<Integer> idList = new ArrayList<>();
                for (MidCollectorKeyword mid : midCollectorKeywordSet) {
                    idList.add(mid.getKeyword_id());
                }
                List<KeywordIndex> keywordIndexList = keywordIndexMapper.selectKeywordInId(idList);
                if (CollectionUtils.isEmpty(keywordIndexList)) {
                    return new HashSet<>();
                } else {
                    Set<String> keywordSet = new HashSet<>(keywordIndexList.size());
                    for (KeywordIndex tag : keywordIndexList) {
    ...
    private Set<KvIndex> getkvTagSet(String applicationName) {
        ...
        if (collector == null) {
            ...
            Set<MidCollectorKv> midCollectorKvSet = kvIndexMapper.selectMidByCollectorId(collector.getId());
            if (CollectionUtils.isEmpty(midCollectorKvSet)) {
                return new HashSet<>();
            } else {
                List<Integer> idList = new ArrayList<>();
                for (MidCollectorKv mid : midCollectorKvSet) {
                    idList.add(mid.getKv_id());
                }
                List<KvIndex> kvIndexList = kvIndexMapper.selectKvInId(idList);
                if (CollectionUtils.isEmpty(kvIndexList)) {
                    return new HashSet<>();
                } else {
                    Set<KvIndex> kvSet = new HashSet<>(kvIndexList.size());
                    for (KvIndex tag : kvIndexList) {
                        kvSet.add(tag);
                    }
                    return kvSet;
                }
